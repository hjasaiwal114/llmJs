...
const llm = new OpenAI({ temperature: O });
...
text: {input}
`
const reviewPromptTemplate1 = new PromptTempelate({
    template: responseTemplate1,
    inputVaiables: ["input"],
});

const reviewPromptTemplate2 = new PromptTemplates({
    template: responseTemplate2,
    inputVariable: ["input"],
});

const reviewChain1 = new LLMChain({ llm, prompt: reviewPromptTemplete1 });
...
const result = await overallChain.run({
    input: "I ordered Pizza Salami and it was awfull",
});
